...
function StatusBadge({ status, T }) {
  const occupied = (status || "").toLowerCase() === "occupied";
  const a = occupied ? ACCENT.green : ACCENT.orange;
  return (
    <span style={{
...
function FlatRow({ flat, T, delay = 0 }) {
  const [visible, setVisible] = useState(false);
  ...
  return (
    <div
      ...
      style={{
        display: "grid",
        gridTemplateColumns: "1.2fr 1fr 0.8fr 1.5fr 1fr 32px",
        alignItems: "center",
        gap: 12,
        padding: "14px 18px",
        borderRadius: 10,
    ...
    >
      ...
      <span style={{ fontSize: 13, color: T.inkSoft }}>{flat.buildingName}</span>
      <span style={{ fontSize: 13, color: T.inkSoft }}>Floor {flat.floorNumber}</span>
      <span style={{ fontSize: 12.5, color: T.inkSoft }}>
        {flat.residentName || "—"}
      </span>
      <StatusBadge status={flat.occupancyStatus} T={T} />
...
function FullSpinner({ T }) {
  return (
    <div style={{ textAlign: "center", padding: "48px 20px" }}>
      ...
      <p style={{ color: T.inkSoft, fontSize: 13, marginTop: 12 }}>Loading flats…</p>
    </div>
  );
}
...
const Flats = () => {
  ...
  const loadBuildings = async () => {
    try {
      const response = await flatService.getBuildings();
  ...
  const handleCreateUnit = async (e) => {
    e.preventDefault();
    try {
      const response = await flatService.createFlat(newUnitForm);
      if (response.data.success) {
        alert("Unit created successfully!");
        setShowCreateModal(false);
        setNewUnitForm({ building_id: "", floor_number: "", unit_number: "" });
        await loadFlats();
      } else {
        alert(response.data.message || "Failed to create unit");
      }
    } catch (error) {
  ...
  useEffect(() => { loadFlats(); loadBuildings(); }, []);
  ...
  const handleRefresh = () => {
    setRefreshSpin(true);
    loadFlats().then(() => setTimeout(() => setRefreshSpin(false), 600));
  };
  ...
  const filtered = flats.filter((f) => {
    ...
    const matchesStatus =
      statusFilter === "all" ||
      (f.occupancyStatus || "").toLowerCase() === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const totalFlats = flats.length;
  const occupiedFlats = flats.filter((f) => (f.occupancyStatus || "").toLowerCase() === "occupied").length;
  const vacantFlats = totalFlats - occupiedFlats;
  const buildings = new Set(flats.map((f) => f.buildingName)).size;
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ color: T.inkSoft, margin: "6px 0 0", fontSize: 14 }}>
              Manage unit inventory, occupancy, and assignments
            </p>
          </div>
        ...
        <div style={{
          ...
          <StatCard title="Total Flats" value={totalFlats} subtitle="Across all buildings" icon={Home} color="blue" delay={0} T={T} />
          <StatCard title="Occupied" value={occupiedFlats} subtitle="Currently assigned" icon={CheckCircle2} color="green" delay={70} T={T} />
          <StatCard title="Vacant" value={vacantFlats} subtitle="Available units" icon={Users} color="orange" delay={140} T={T} />
          <StatCard title="Buildings" value={buildings} subtitle="With registered units" icon={Building2} color="purple" delay={210} T={T} />
        </div>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {["Flat No.", "Building", "Floor", "Resident", "Status", ""].map((h, i) => (
              <span key={i} style={{
          ...
          <div style={{ padding: "6px 8px" }}>
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "48px 20px" }}>
                <Home size={32} color={T.inkMuted} style={{ marginBottom: 10 }} />
                <p style={{ color: T.inkSoft, fontSize: 13.5, margin: 0 }}>No flats found.</p>
              </div>
            ) : (
              filtered.map((flat, i) => (
                <FlatRow key={flat.unitId || i} flat={flat} T={T} delay={i * 40} />
              ))
            )}
          </div>
        </div>
          ...
          <div style={{
            ...
            <form onSubmit={handleCreateUnit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12.5, color: T.inkMuted }}>
                ...
                <select
                  required
                  value={newUnitForm.building_id}
                  onChange={e => setNewUnitForm({ ...newUnitForm, building_id: e.target.value })}
                  style={{
                    width: "100%",
                    padding: "10px 12px",
                    borderRadius: 8,
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: 6, fontSize: 12.5, color: T.inkMuted }}>
                ...
                <input
                  required
                  type="number"
                  value={newUnitForm.floor_number}
                  onChange={e => setNewUnitForm({ ...newUnitForm, floor_number: e.target.value })}
                  style={{
                    width: "100%",
                    padding: "10px 12px",
                    borderRadius: 8,
